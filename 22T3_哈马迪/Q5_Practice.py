...
def betrothed_pair(n):
    ...
    left = n
    while True:
        right = sum(divisor(left)) - 1
        if sum(divisor(right)) - 1 == left and left != right:
            break
        left += 1

    print(f'The least number >= {n} that is the first member of a Betrothed pair is {left}')
    print(f'The Betrothed itself is ({left}, {right})')
